...
object Huffman {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = chars.distinct.map(char => (char, chars.count(otherChar => char == otherChar)))
   ...
   */
  def until(predicate: List[CodeTree] => Boolean, update: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (predicate(trees)) trees
    else until(predicate, update)( update(trees) )
  }
